var stressTest = (function() {
  ...
  function style(elm, props) {
    forEach.call(Object_keys(props), function(ii) {
      try {
        var name = ii.replace(/\-([a-z])/ig, function(a, l) {
  ...
  function indexElements(state) {
    var all = state.all || getChildren(document),
        ret = {};
    forEach.call(all, function(elm) {
      if (elm.className && !elm.toString().match(/svg/i)) {
        forEach.call(
        filter.call(elm.className.split(whitespace), function(n) {
          return n.length > 0;
        }), function(n) {
          if (!ret['.' + n]) ret['.' + n] = [];
          ret['.' + n].push(elm);
...
/*var l = 0;
  ...
  function bind(elm, name, func) {
    var parts = name.split('.'),
        _func = !elm.attachEvent ? func : function() {
  ...
  function removeClass(elms, className) {
    forEach.call(elms, function(ii) {
      ...
      filter.call((ii.className || '').split(whitespace), function(jj) {
        return jj != className;
      })).join(' ');
    });
  }
  ...
  function removeSelector(elms, selector) {
    ...
    else if (which === '#') forEach.call(elms, function(elm) {
      elm.attributes.removeNamedItem('id');
    });
  }
  ...
  function testSelector(selector, state, finished) {
    ...
    if (state.beforeTest) state.beforeTest({
      elms: elms,
      selector: selector
    });
  ...
  function extend(a, b, copy) {
    ...
    forEach.call(Object_keys(a), function(key) {
      if (copy || !b.hasOwnProperty(key)) b[key] = a[key];
    });
    return b;
  }
  ...
  function formatNumber(number, leading, trailing) {
    ...
    while (parts[1].length < trailing) parts[1] += '0';
  ...
  function showReport(state, report) {
    ...
    forEach.call(worst, function(ii) {
      log += '<tr><td>Removing <strong style="font:12px monospace">' + ii + '</strong></td><td style="text-align:right; font:12px monospace">' + state.results[ii].length + '</td><td style="text-align:right; font:12px monospace">' + state.results[ii].children + '</td><td style="text-align:right">' + (state.results[ii].delta < 0 ? '<span style="color:red">saves</span>' : '<span style="color:green">adds</span>') + '</td><td style="text-align:right; font:12px monospace">' + formatNumber(Math.abs(state.results[ii].delta) / state.times) + 'ms</td><td style="text-align:right; font:12px monospace">' + formatNumber(state.results[ii].time / state.times) + 'ms</td></tr>\n';
    });
    log += '</table><hr/><table><tr><td style="text-align:right">Selectors Tested:</td><td style="font:12px monospace">' + all.length + '</td></tr>' + '<tr><td style="text-align:right">Baseline Time:</td><td style="font:12px monospace">' + formatNumber(state.baseTime / state.times) + 'ms</td></tr>' + '<tr><td style="text-align:right">Num. Tests:</td><td style="font:12px monospace">' + state.times + '</td></tr>';
    ...
    forEach.call(
    getChildren(report, 'td th'), function(td) {
      style(td, {
        padding: 1,
        verticalAlign: 'top',
        whiteSpace: 'nowrap',
  ...
  stressTest.bookmarklet = function() {
    ...
    var report = reportHolder.doc.createElement('div'),
        ...
        beforeTest: function(e) {
          ...
          report.innerHTML = 'Testing <strong>' + e.selector + '</strong><br/>' + l + ' test' + (l === 1 ? '' : 's') + ' remain';
        },
